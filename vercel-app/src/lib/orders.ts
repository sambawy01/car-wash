 ...
 * - Status flow is strictly ordered → shipped → delivered, enforced in
 *   `updateOrderStatus`; every transition is appended to `statusHistory`.
 */

export const ORDER_STATUSES = ["ordered", "shipped", "delivered"] as const;
export type OrderStatus = (typeof ORDER_STATUSES)[number];

/** The only legal next step for each status (terminal: delivered). */
...
/**
 * Advance an order's status. Only the forward transitions
 * ordered→shipped→delivered are legal; anything else returns a typed
 * `invalid-transition` error so the API layer can answer 400.
 */
